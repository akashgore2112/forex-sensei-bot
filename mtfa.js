...
class MTFA {  
  ...
  static async analyze(pair = "EUR/USD") {  
    try {  
      ...
      // ✅ Calculate indicators for each timeframe (CHANGED: calculateCurrent instead of calculateAll)
      const dailyIndicators = await SwingIndicators.calculateCurrent(daily);  
      const weeklyIndicators = await SwingIndicators.calculateCurrent(weekly);  
      const monthlyIndicators = await SwingIndicators.calculateCurrent(monthly);  
      ...
      // ✅ Final Result (safe + extended with raw candles)  
      const result = {  
        pair,  
        ...
        // 🔥 Extra for Phase 2 (safe to ignore in Phase 1)  
        dailyCandles: daily,  
        weeklyCandles: weekly,  
        monthlyCandles: monthly,  
